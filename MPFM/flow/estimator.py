...
from MPFM.flow.dit import DiTConVBlock
...
class Decoder(nn.Module):
    def __init__(self, hidden_channels, out_channels, filter_channels, dropout=0.1, n_layers=1, n_heads=4, kernel_size=3, gin_channels=0):
        super().__init__()
    ...
    def forward(self, x, mask, mu, t, spks, cond):
        ...
        x = pack([x, mu], "b * t")[0]
        if spks is not None:
            spks_rep = repeat(spks, "b c -> b c t", t=x.shape[-1])
            x = pack([x, spks_rep], "b * t")[0]
        if cond is not None:
            x = pack([x, cond], "b * t")[0]
        for block in self.blocks:
